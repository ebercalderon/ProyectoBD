...
$db = getDbInstance();
$select = array('CodigoBien', 'Descripcion', 'UM', 'Cantidad', 'Monto', 'estado');


$chunk_size = 100;
$offset = 0;

$data = $db->withTotalCount()->get('bien');
$total_count = $db->totalCount;

$handle = fopen('php://memory', 'w');

fputcsv($handle, $select);
$filename = 'export_bienes.csv';
...
for ($i = 0; $i < $num_queries; $i++) {
    ...
    $rows = $db->get('bien', array($offset, $chunk_size), $select);
